...
| **v0.34** | Three ways in, and the cost of the way through. The check on where a detector may talk read a *different hostname* than the request went to — three Unicode characters, or an address in `inet_aton` notation, and the document went to a public host as internal. A failed `protect` left the values on disk in the clear. A settings file nobody named could point the detection pass anywhere. Then the cost: memory had never been measured at all, and the first measurement found **157 bytes of peak allocation per input character**, half of it an offset map recording that character 40,000 is at character 40,000 — now 68 on mixed text and 19 on English prose. A fourth quadratic, which needed a document with *two languages in it* to see. Plus the seventeen recogniser tests that had never run anywhere, and the brief for a corpus somebody else wrote — [docs/corpus-brief.md](docs/corpus-brief.md), scope and all. Still open, and both waiting on evidence rather than effort: the bare Chinese mobile and the unanchored English name. |
| **v0.35** | What the measurements point at next, in the order they earned. **Peak allocation is now the findings themselves** — about 400 bytes an entity, retained because the caller asked for them — plus transient detection state; shaving constants further is worth less than deciding whether a document has to be held whole at all, which is the proxy's question rather than the library's. **The scaling suite had one blind spot and probably has others**: every shape in it repeated a single unit, and the quadratic that hid there needed alternation to see, so the shapes get generated rather than listed. **The two contract decisions `1.0` cannot ship without** — when `protection-scope/1` freezes, and whether identity folds case as it folds width. And whatever the commissioned corpus says, which is the only thing that can settle the two leaks above. |
