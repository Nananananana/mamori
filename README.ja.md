...
パターン規則が拾うのは、ベンダー接頭辞、PEM ブロック、DB スキーム、そして値の隣にある
キーワードである。そのどれもない鍵——素の 40 桁 hex トークン、ランダムなセッション ID、
`Authorization: Bearer a3f9c2e1…`——は最初のリリースから文書化された穴で、
**既定では今もそのまま**である:

```python
MamoriConfig(secrets="patterns")  # 既定。規則以外は何もしない
MamoriConfig(secrets="entropy")  # 加えて、文字の散らばり方を測る
```

`"entropy"` は `detect-secrets` / `gitleaks` / `trufflehog` が共有する Shannon エントロピー
のパスを、規則の**後**に、アンカーのある何かが既に取った区間を除いて走らせる。隣の語が
確信度を決める——*key*、*token*、*bearer*、鍵、密钥 なら `MEDIUM`、*commit*、*sha*、
*hash* なら `LOW`——ので、`min_confidence=0.6` にすると「何かが秘密と呼んだ候補」だけが残る。

**既定にしない理由は 1 つ。** 見つけたものは `API_KEY` として報告され、既定ポリシーは
資格情報を置換ではなく**ブロック**する。ここでの誤検出は余計なプレースホルダではなく、
**要求の停止**である。エントロピーは鍵とコンテンツハッシュを区別できないので、プロンプト
中のコミット ID が拒否された文書になる。それでも素の鍵が外に出るより安い、と判断した
運用が自分で入れる設定であって、ライブラリが代わりに決めることではない。

同梱コーパスでは**このトレードを測れない**: 167 件中、鍵の形をした 20 文字以上の並びは 8 本、
生成されたものは 0 本で、この README のすべての数字は ON でも同一だった。コストは合成
ケースからの記述であり、何が決着させるかは [open-questions.md](docs/open-questions.md) にある。

アルゴリズムは名前で、名前はレジストリである: 4 つ目——候補ごとに一問だけ訊くローカル
モデル、既知の漏洩鍵のフィルタ——は `register_secret_algorithm("name", factory)` と設定値
1 つで足り、編集ではない。`mamori privacy` はどれが動いていて、何がブロックされるかを報告する。
