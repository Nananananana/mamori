...
| | |
|---|---|
| **v0.9** | 数字の土台を作り直す。より大きく難しいデータセット、自分のラベル付きデータで測る方法の整備、そして `v0.7` で開いた問い（8Bより大きいモデルで表は変わるか）に答える。 |
| **v0.10** | 代替値方式（`田中太郎` → `山田一郎`）をポリシー選択肢として追加。 |
| **v0.11** | opt-inの暗号化永続ストア（保持期間は「機械」ではなく「明示されたルール」として）、Presidioアダプタ。 |
| **v1.0** | 機能ではない。APIの安定、約束スイートを仕様とすること、そして「実測」と呼ぶに値するデータに裏付けられた数字。 |

この表の理由と、**あえて計画に入れていないもの**は
[docs/proposals/0001](docs/proposals/0001-the-road-to-1-0.md) にある。

次はモデル層の測定である。このライブラリで唯一、品質が「測定」ではなく
「主張」に留まっている部分であり、それを決着させる基盤は `v0.2` から揃っている。
