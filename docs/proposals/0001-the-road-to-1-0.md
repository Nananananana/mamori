...
**Status:** current plan, revised after 0.7.0
...
## What the first seven versions actually established
...
## What 0.7 changed about the plan
...
### 0.9 — The evidence under the numbers

Every claim in `SECURITY.md` rests on 123 synthetic sentences. This version
makes that foundation carry the weight it is being asked to bear:

- larger bundled sets, and harder ones — the cases that are currently absent
  because they are difficult to invent, not because they are rare;
...
- the open question from 0.7, answered rather than asserted: does a model above
  8B change the table? The harness exists; the run takes one command.

### 0.10 — Surrogate values
...
### 0.11 — Deployment: persistence and integration
...
| ADR-0062 retention is a rule, not a machine | 0.11 | the encrypted store |
| ADR-0069 the gate is evidence, not a word list | — | why there is no stoplist |
