...
| | |
|---|---|
| **v0.2** | OpenAI 兼容的本地代理，现有应用只改 `base_url` 即可接入。同时为各语言准备带标注的评测集，让检测精度成为数字而不是意见。 |
| **v0.3** | 词典与规则检测器、Presidio 适配器、可选启用的加密持久化存储。 |
| **v0.4** | 本地模型检测作为可选的深度扫描层，用于模式无法覆盖的情况——首先是无锚点的英文人名和中文名。 |
| **v0.5** | 替身值（`张伟` → `王强`）作为策略选项，用于占位符对回答质量损害过大的场景。 |

代理排在第二位是刻意的。没有人会为了采用一个库去重写一个正在运行的应用，
